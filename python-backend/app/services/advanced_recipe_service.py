...
from app.core.llm.advanced_prompts import AdvancedRecipePrompts
...
class AdvancedRecipeService:
    ...
    def _combine_multiple_json_blocks(self, content: str) -> Dict:
        ...
        # Extract individual recipe blocks more carefully
        recipe_blocks = re.findall(r'\{\s*"recipes"\s*:\s*\[\s*\{[^}]*\}\s*\]\s*\}', content, re.DOTALL)
        ...
        # If no recipes found with first method, try extracting recipe data directly
        if not all_recipes:
            print(f"🔧 [PYTHON] Trying direct recipe extraction...")
            recipe_names = re.findall(r'"recipe_name"\s*:\s*"([^"]+)"', content)
            
            for i, name in enumerate(recipe_names):
                # Extract ingredients for this recipe
                ingredients_match = re.search(rf'"recipe_name"\s*:\s*"{re.escape(name)}"[^}}]*"inventory_items_used"\s*:\s*\[([^\]]+)\]', content, re.DOTALL)
                steps_match = re.search(rf'"recipe_name"\s*:\s*"{re.escape(name)}"[^}}]*"steps"\s*:\s*\[([^\]]+)\]', content, re.DOTALL)
                ...
                if ingredients_match:
                    ...
                    ingredient_matches = re.findall(r'\{"name"\s*:\s*"([^"]+)"\s*,\s*"quantity"\s*:\s*"([^"]+)"\s*,\s*"unit"\s*:\s*"([^"]+)"\}', ingredients_text)
                    for ing_name, ing_qty, ing_unit in ingredient_matches:
                        recipe["inventory_items_used"].append({
                            "name": ing_name,
                            "quantity": ing_qty,
                            "unit": ing_unit
                        })
                ...
                if steps_match:
                    ...
                    recipe["steps"] = step_matches
                
                all_recipes.append(recipe)
                print(f"✅ [PYTHON] Extracted recipe: {name}")
        
        # If still no recipes found, try manual extraction
        if not all_recipes:
            print(f"❌ [PYTHON] No recipes found in JSON blocks, trying manual extraction")
